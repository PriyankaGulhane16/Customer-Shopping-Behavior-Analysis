# Customer-Shopping-Behavior-Analysis
Data Analytics Project showcasing customer behavior analysis using Python, SQL, Power BI Dashboard
📝 Overview

This project demonstrates a complete end-to-end data analytics workflow, from raw dataset to interactive business dashboard.

The project includes:

Loading and analyzing data using Python

Performing Exploratory Data Analysis (EDA)

Cleaning and transforming data

Writing analytical queries in SQL (PostgreSQL)

Building an interactive dashboard in Power BI

The goal of this project is to showcase practical data analysis skills used in real-world business environments.

📂 Dataset

The dataset used in this project contains structured data with multiple features relevant to business analysis (e.g., sales, customers, products, dates, categories).

Key characteristics:

Structured tabular data (CSV format)

Multiple numerical and categorical columns

Contains missing values and inconsistencies

Suitable for aggregation, trend analysis, and KPI tracking

🛠 Tools & Technologies

Python

pandas

SQL

PostgreSQL

Data Visualization

Power BI

Jupyter Notebook

PostgreSQL Server

🔎 Project Steps
1️⃣ Data Loading (Python)

Imported dataset using pandas

Inspected structure using .head(), .info(), .describe()

Identified missing values and data types

2️⃣ Exploratory Data Analysis (EDA)

Checked distributions of numerical variables

Analyzed categorical variables

Identified correlations between features

Visualized trends using:

Bar charts

Line plots

Heatmaps

3️⃣ Data Cleaning

Handled missing values

Removed duplicates

Standardized column names

Converted data types

Created calculated columns where needed

4️⃣ SQL Analysis (PostgreSQL)

Imported cleaned dataset into PostgreSQL

Wrote SQL queries for:

Aggregations (SUM, AVG, COUNT)

Grouped analysis (GROUP BY)

Joins (if multiple tables)

Date-based analysis

Top-N analysis

Example queries included:

Monthly revenue trends

Top-performing categories

Customer segmentation metrics

5️⃣ Power BI Dashboard

Connected Power BI to PostgreSQL

Created data model and relationships

Designed interactive visuals:

KPI cards

Line charts (trend analysis)

Bar charts (category comparison)

Filters and slicers

📊 Dashboard Highlights

The Power BI dashboard provides:

📈 Revenue trends over time

🏆 Top-performing products/categories

👥 Customer insights

📍 Regional performance analysis

📊 Key KPIs (Total Sales, Profit, Orders, etc.)

The dashboard is interactive and allows dynamic filtering by date, category, or region.

📈 Results & Insights

Through this analysis:

Identified key revenue drivers

Discovered high-performing categories

Detected seasonal trends

Highlighted potential improvement areas

This project demonstrates the ability to:

Work with raw data

Perform structured analysis

Write efficient SQL queries

Translate data into business insights

Build professional dashboards
